  ...
  function aniB(parentCanvas) {
    ...
    for (let i = 0; i < rows; i++) {
      for (let j = 0; j < cols; j++) {
        const x = j * (circleRadius * 2 + padding) + circleRadius + padding;
        const y = i * (circleRadius * 2 + padding) + circleRadius + padding;
        circles.push({ x, y, radius: circleRadius, clicks: 0 });
      }
    }
